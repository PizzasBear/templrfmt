...
impl Printer {
    pub fn item(&mut self, item: &Item) {
        if let Some(span) = item.begin_span() {
    ...
    fn item_fn(&mut self, item: &ItemFn) {
        ...
        self.flush_comments(
            ...
            item.attrs.is_empty() && item.block.stmts.is_empty(),
            true,
        );
        self.offset(-INDENT);
        self.end();
    ...
    fn item_foreign_mod(&mut self, item: &ItemForeignMod) {
        ...
        self.flush_comments(
            item.brace_token.span.close(),
            item.attrs.is_empty() && item.items.is_empty(),
            true,
        );
        self.offset(-INDENT);
        self.end();
    ...
    fn item_impl(&mut self, item: &ItemImpl) {
        ...
        self.flush_comments(item.brace_token.span.close(), item.items.is_empty(), true);
        self.offset(-INDENT);
        self.end();
        self.word("}");
        self.hardbreak();
    ...
    fn item_mod(&mut self, item: &ItemMod) {
        ...
        if let Some((brace, items)) = &item.content {
            ...
            self.flush_comments(
                brace.span.close(),
                item.attrs.is_empty() && items.is_empty(),
                true,
            );
            self.offset(-INDENT);
            self.end();
    ...
    fn foreign_item(&mut self, foreign_item: &ForeignItem) {
        if let Some(span) = foreign_item.begin_span() {
            self.flush_comments(span, false, false);
        }
    ...
    fn trait_item(&mut self, trait_item: &TraitItem) {
        if let Some(span) = trait_item.begin_span() {
            self.flush_comments(span, false, false);
        }
    ...
    fn trait_item_fn(&mut self, trait_item: &TraitItemFn) {
        ...
        if let Some(block) = &trait_item.default {
            ...
            self.flush_comments(
                ...
                trait_item.attrs.is_empty() && block.stmts.is_empty(),
                true,
            );
            self.offset(-INDENT);
            self.end();
    ...
    fn impl_item(&mut self, impl_item: &ImplItem) {
        if let Some(span) = impl_item.begin_span() {
            self.flush_comments(span, false, false);
        }
    ...
    fn impl_item_fn(&mut self, impl_item: &ImplItemFn) {
        ...
        self.flush_comments(
            impl_item.block.brace_token.span.close(),
            impl_item.attrs.is_empty() && impl_item.block.stmts.is_empty(),
            true,
        );
        self.offset(-INDENT);
        self.end();
    ...
    fn signature(&mut self, signature: &Signature) {
        ...
        self.flush_comments(
            signature.paren_token.span.close(),
            signature.inputs.is_empty() && signature.variadic.is_none(),
            true,
        );
        self.offset(-INDENT);
        self.end();
    ...
    fn fn_arg(&mut self, fn_arg: &FnArg) {
        if let Some(span) = fn_arg.begin_span() {
            self.flush_comments(span, false, false);
        }
    ...
    fn variadic(&mut self, variadic: &Variadic) {
        if let Some(span) = variadic.begin_span() {
            self.flush_comments(span, false, false);
        }
