...
impl Printer {
    pub fn expr(&mut self, expr: &Expr) {
        if let Some(span) = expr.begin_span() {
            self.flush_comments(span, true, true);
        }
    ...
    fn expr_for_loop(&mut self, expr: &ExprForLoop) {
        ...
        self.flush_comments(
            expr.body.brace_token.span.close(),
            expr.body.stmts.is_empty(),
            true,
        );
        self.offset(-INDENT);
        self.end();
    ...
    fn expr_if(&mut self, expr: &ExprIf) {
        ...
        if let Some((_else_token, else_branch)) = &expr.else_branch {
            ...
            self.flush_comments(
                expr.then_branch.brace_token.span.close(),
                expr.then_branch.stmts.is_empty(),
                true,
            );
            self.offset(-INDENT);
            self.word("}");
    ...
    fn arm(&mut self, arm: &Arm) {
        ...
        if let Expr::Block(body) = body {
            ...
            self.flush_comments(
                body.block.brace_token.span.close(),
                body.block.stmts.is_empty(),
                true,
            );
            self.offset(-INDENT);
            self.end();
    ...
    pub fn small_block(&mut self, block: &Block, attrs: &[Attribute]) {
        ...
        if attr::has_inner(attrs) || !block.stmts.is_empty() {
            ...
            self.flush_comments(block.brace_token.span.close(), block.stmts.is_empty(), true);
            self.offset(-INDENT);
        }
        self.word("}");
    }
